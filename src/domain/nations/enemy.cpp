...
enemy::enemy(std::string _name, int _mindamage, int _maxdamage,
             double _attackspersecond, int _hitpoints, int _grantedXP,
             int _range, int _movement, bool _boss,
             std::shared_ptr<nation> _nation, int _oppertunitycosts) {
  name = _name;
...
double enemy::getattackspersecond() { return attackspersecond; }

int enemy::getHitpoints() { return hitpoints; }

int enemy::getRange() { return range; }
int enemy::getMovement() { return movement; }
int enemy::getOppertunity() const { return oppertunitycosts; }

bool enemy::getBoss() { return boss; }
...
engine::math::box2_t enemy::get_box() const {
  return engine::math::box2_t(engine::math::vec2_t(1, 1),
                              engine::math::vec2_t(1, 1));
}

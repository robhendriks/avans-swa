...
namespace domain {
    namespace drawable {
        ...
        public:
            ...
            // can be overwritten in case you don't want to load the textures of the current dragable_field_object because there is a chance
            // you won't draw them but only of a child dragable_field_object. (if overwritten no call will be made to draw_object)
        ...
        protected:
            ...
            // load texture of current dragable_field_object in case its not loaded in yet. (this will fill texture field or give a exception
            void load_texture_if_not_loaded_yet(engine::graphics::texture_manager &texture_manager);

        private:
            // basic implementation is to draw the current dragable_field_object (needs to be overwritten for also drawing children)
